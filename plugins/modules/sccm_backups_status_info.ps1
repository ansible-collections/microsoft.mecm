...
# Test CM Site drive availability
if (-not (Test-CMSiteDrive -SiteCode $siteDrive.Name)) {
    $module.FailJson("CMSite drive $($siteDrive.Name) is not available")
}
...
try {
    ...
    $backupTask = Get-CMSiteMaintenanceTask -TaskName "Backup SMS Site Server" -ErrorAction Stop
}
catch {
